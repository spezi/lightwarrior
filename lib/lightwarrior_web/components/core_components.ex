defmodule LightwarriorWeb.CoreComponents do
  @moduledoc """
  ...
  def led_instance(assigns) do
    ~H"""
      <.link patch={~p"/hyperion/ledmappings/#{@instance["instance"]}/edit"}>
        <button  class={
                "m-1 p-2 rounded-full text-xs font-semibold text-white shadow-xl
                #{if @status, do: "bg-green-600 ", else: "bg-red-600 "}
                #{if @selected == @instance["instance"], do: "ring-offset-2 ring-2 ring-emerald-400 ", else: "" }
              "}
        ><%= @title %>
        </button>
      </.link>
  ...
  @doc """
  Renders a little css spinner with an svg.
  TODO
  ## Examples
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LightwarriorWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LightwarriorWeb.Gettext, "errors", msg, opts)
    end
  end
